...
** Element
*** Tag
Is a keyword or term assigned to a piece of information. This kind of metadata helps describe an item and allows it to be found again by browsing or searching.
...
**** input
The ~<input>~ (inline) tag specifies an input field where the user can enter data. This element is the most important form element. This element can be displayed in several ways, depending on the type attribute.
...
#+begin_src html
  ...
  <input type="button">
  <input type="checkbox" | "color" | "date" | "file" | "number" |
         "password" | "radio" | "range" | "datetime-local" |
         "email" | "hidden" | "image" | "month" | "reset" |
...
*Node*: ~Attr~ objects inherit the ~Node~ interface, but since *they are not actually child nodes of the element they describe*, the DOM *does not consider them part of the document tree*.
...
[[https://javascript.info/size-and-scroll][Javascript.Info]]
...
#+begin_src
  ...
  scrollHeight  -----  ├───────────┤                │         |▲│
    ...
    -----              └-----------┘                └-----------┘
         ...
         scrollHeight = innerHeight + MAX(scrollY)
...
*** Based on subset of the elements (Class)
...
#+begin_src css
  ...
  /* select the items that has 'one' and 'two' class, e.g. 'class="one two" */
  .one.two { }
...
*** Based on state (Pseudo Classes) (:, :not())
**** Brief
Is a keyword added to a selector that *specifies a special state (behavior) of the selected element(s)*. Pseudo-classes are used in CSS selectors to permit formatting based on information that is not contained in the document tree.
...
*w3schools*: is used to apply the same style to all the elements inside the parentheses, at the same time. The ~:is()~ pseudo-class can be very useful when dealing with HTML sections and headings.
...
*ManzDev*: ~:is()~ es un reemplazo práctico de la agrupación de selectores mediante comas.

Con la pseudoclase ~:is()~, se calcula la especificidad sumando la especificidad más
alta de sus parámetros.
...
*w3schools*: the CSS ~:where()~ pseudo-class is used to apply the same style to all the elements inside the parentheses, at the same time.
...
*Note*: ~p:nth-child(n)~ represents *every* ~<p>~ *element in a group of siblings*. This selects the same elements as a simple ~p~ selector (although with a higher specificity).
...
*Note*: the [[:nth-of-type()]] pseudo-class select the element that is the ~nth~ child, *of the same type (tag name)*, of its parent.
...
#+begin_src css
  ...
  /* All p elements whose index is a multiple of 3. Then we subtract 1 */
  p:nth-child(3n-1) {
    background-color: red;
  }
...
- :nth-child(5n) :: represents elements *5* ~[=5×1]~, *10* ~[=5×2]~, *15* ~[=5×3]~, etc. The first one to be returned as a result of the formula is *0* ~[=5x0]~, resulting in a no-match, since the elements are indexed from 1, whereas ~n~ starts from 0. This may seem weird at first, but it makes more sense when the ~b~ part of the formula ~(an + b)~.

***** p:nth-child() vs p:nth-of-type()

#+begin_src css

  /* Look for the third element; if that element is p, color it in ... */
  p:nth-child(3n) {
    background: red;
  }

  /* Look for the third element that is of type p, and color it in ... */
  p:nth-of-type(3n) {
    background: yellow;
  }
...
*w3schools*: the CSS ~:nth-of-type(n)~ pseudo-class matches every element that is the nth child, of the same type (tag name), of its parent.

*Note*: the [[:nth-child()]] pseudo-class to select the element that is the ~nth~ child, *regardless of type*, of its parent.

*Note*: : ~nth-of-type()~ pseudo-class has the same syntax that ~:nth-child()~.
...
Difference between ~p:nth-child(3n)~ and ~p:nth-of-type(3n)~
...
*** Based on parts of element (Pseudo Elements) (::)
**** Brief
...
The ~::before~ selector *inserts something before the content* of each selected element(s). Use the [[Content property][content]] property to specify the content to insert. Use the ~::after~ selector to insert something after the content.

*Note*: if the ~content~ property is not specified, has an invalid value, or has normal or none as a value, then the ~::before~ pseudo-element is not rendered. It behaves as if ~display: none~ is set.
...
#+begin_src css
  ...
  /* ManzDev: cuando se utiliza el pseudo elemento ::before or ::after se */
  /* tiene que añadir un content para poder añadir información y se vea */
  /* visualmente. */
  element::before {
    content: "";
    ...
...
#+begin_src html
  ...
  +----------------------+       Some start text....
  |  ::before            |       <div class="in">
  |  START Some start    |         <div class="in-in"></div>
  |  text...             |       </div>
  |  +--------------+    |       Some end text....
  |  | div          |    |     </div>
  |  | +----------+ |    |
  ...
  |  | +----------+ |    |       .container::before {
  |  +--------------+    |         content: "START";}
  |  Some end text...END |       .container::after {
  |  ::after             |         content: "END";}
  +----------------------+     </style>
...
*** Based on relationship to other elements (>, ~, :has())
**** Child combinator (>)
...
*w3schools*: is used to apply styles for an element within the context of another element.
...
** Properties
*** Definition
Is a specific characteristic that you can use to style HTML elements, such as color, font-size, or margin. Each property has a name and a value that determines how the element will be displayed on a webpage.

*MDN*: A CSS property is a parameter used in a CSS declaration that lets you style certain aspects of selected elements.
...
- revert :: resets the property to its inherited value if it inherits from its parent or to the default value established *by the user agent's stylesheet* (or by user styles, if any exist) (browser's default styling rather than the defaults applied to that property). This value acts like unset in many cases.
...
A custom property prefixed with two dashes begins with ~--~ , followed by the property name (e.g., ~--my-property~ ), and a property value that can be any valid CSS value. Like any other property, this is written inside a ruleset.
...
#+begin_src css
  ...
  /* 🎮 Game News: A new inFamous is not planned */
...
- The box will break onto a new line. Always start on a new line
...
- The box will not break onto a new line. Do not start on a new line
- The ~width~ and ~height~ properties will not apply. Only use as much horizontal space as required by the content
- Margins will work horizontally, but not vertically
...
**** Inner display type (flow layout, grid, flex)
...
*Note*: [[object-fit]] property sets how the content of a replaced element, such as an ~<img>~ or ~<video>~, should be resized to fit its container.
...
There are five possible values of the ~position~ property. ~static~ positioning is the default in normal flow. *If an item is positioned in any way other than* ~static~ ( ~fixed~ , ~absolute~ , ~relative~ , or ~sticky~ ), then the further properties ~top~, ~bottom~, ~left~, and ~right~ *are used to specify offsets and positions*. The element having position static is not affected by the ~top~, ~bottom~ , ~left~ or ~right~ properties.
...
*Eloquent*: when the ~position~ style property is set to ~relative~, the element still takes up space in the document, but now the ~top~ and ~left~ style properties can be used to move it *relative to that normal place*.
...
#+begin_src css
  ...
  /* Ejemplo de fondo fijo (si hay scroll sigue estando fijo) */
  .container {
    background: red;
    position: fixed;
    inset: 0;
...
*MDN*: The following things affect the counter values on an element:
- Counters are ~inherited~ from the parent element or received from a previous sibling.
- New counters are instantiated using ~counter-reset~ property.
- Counters are incremented using ~counter-increment~ property.
- Counters are directly set to a value using the ~counter-set~ property.
...
#+begin_src css
  ...
  /* Syntax  */
  /* drop-shadow(required required optional optional) */
...
- <basic-shape> :: a shape whose size and position is defined by the ~<geometry-box>~ value. If no geometry box is specified, the ~border-box~ will be used as the reference box. One of:
  ...
  - [[circle() ellipse()][circle()]]: defines a circle using a radius and a position.
  - [[circle() ellipse()][ellipse()]]: defines an ellipse using two radii and a position.
  - [[polygon()]]: defines a polygon using an SVG filling rule and a set of vertices.
  - [[path()]]: defines a shape using an optional SVG filling rule and an SVG path definition.
  - [[rect()]]: defines a rectangle using the specified distances from the edges of the reference box.
  - [[shape()]]: defines a shape using an optional SVG filling rule and shape commands for lines, curves, and arcs.
  - [[xywh()]]: defines a rectangle using the specified distances from the top and left edges of the reference box and the specified width and height of the rectangle.
...
#+begin_src css
  ...
  /* <geometry-box> values */
  clip-path: margin-box | border-box | padding-box |
             content-box | fill-box | stroke-box | view-box;
...
#+begin_src
  ...
  (INS)
  Syntax:
  circle(radio, position)

  Default:
  circle(closest-side at 50% 50%)

  The percentage radius refers to the width of the main frame.
  The origin position (0,0) is at the top left
...
**** font-family
Takes font name as values, in order of preference. At the end of the list it is always convenient to put a generic family name.
...
**** text-align
Sets the horizontal alignment of the inline-level content inside a block element or table-cell box. This means it works like [[vertical-align]] but in the horizontal direction.
...
*** Backgrounds
**** background
*Shorthand* property enables you to declare all CSS background properties in a single declaration, which are used to add background effects for elements. The background lies underneath the content of an element. When you have multiple, comma-separated background values, *each is a background layer that is painted on top of the previous layers*. If the top layer contains no transparent areas, this is the only layer that will be visible. The last layer is the bottom layer.
...
- [[background-image]]  :: specifies an *image or gradient* to use as the background of an element
- [[background-position]]  :: sets the *starting position* of a background image
- [[background-size]] :: specifies the *size* of the background images
- [[background-repeat]]  :: sets *if/how* a background image will be *repeated*
- [[background-attachment]]  :: sets *whether* a background image *scrolls* with the rest of the page, or is fixed
- [[background-clip]] ::  *whether* an element's background *extends underneath its* border box, padding box, or content box
- [[background-origin]] :: specifies the *origin position* (the background *positioning area*)
- [[background-color]]  :: sets the background *color* of an element

| /PROPERTY/              | /EXAMPLE/     | /DESCRIPTION/                 |
|-----------------------+-------------+-----------------------------|
| background-image      | radial-g () | creates a radial gradient   |
| background-position   | 1px 2px     | 1px from top 2px from left  |
| background-size       | 1% 2%       | 1% tall and 2% wide         |
| background-repeat     | repeat-x    | repeat horizontally         |
| background-attachment | fixed       | not scroll with the page    |
| background-clip       | padding-box | how far extend within elem  |
| background-origin     | content-box | start from upperleft corner |
| background-color      | red         | set color red               |

#+begin_src css

  /* Default values */
  /* image | position | size | repeat |attachment |clip | origin | coler */
  background: none 0% 0% auto repeat scroll border-box padding-box transparent;
  /* Syntax */
  background: image position / size repeat attachment clip origin color;


  /* Be careful with the shorthand property: */
  background: orange;
  /* equivalent to  */
  ...
  /* any of these same shorthands only paints the red-blue gradient */
  background: linear-gradient(red, blue)   orange;
  background: orange   linear-gradient(red, blue);
  /* this paints the orange-blue gradient (color is underneath gradient) */
  background: orange   linear-gradient(transparent, blue);
...
**** background-image
Sets *one or more background images or gradients* on an element.

By default, the image is repeated so it covers the entire element.

*Note*: it's a good idea to specify a background image and a background color at the same time, in case image fails to load.

*Note*: the background image can also be set for specific elements, like the ~<p>~ element.
...
#+begin_src css
  ...
  /* background image and color at the same time, in case fails to load */
  body {
...
A ~<position>~. A position *defines an x/y coordinate*, to place an item *relative to the edges of an element's box*. It can be defined using one to four values:
...
*Note (ManzDev)*: por defecto, una imagen de fondo toma automáticamente el tamaño de la imagen (que podría ser demasiado grande). Para no tener que modificar la imagen original de forma manual con un editor de imágenes, podemos utilizar esta propiedad y ajustarla a nuestro agrado mediante CSS.
...
*w3schools*: sets *whether* a background *image scrolls with the rest of the page, or is fixed*.

*Values*:
- fixed :: the background is *fixed relative to the viewport* (background *image will not scroll with the page*). Even if an element has a scrolling mechanism, the background doesn't move with the element. If set, the [[background-origin]] property is ignored.
- local :: the background is *fixed relative to the element's contents* (background *image will scroll with the element's contents*). If the element has a scrolling mechanism, the background scrolls with the element's contents, and the background painting area and background positioning area are relative to the scrollable area of the element rather than to the border framing them.
- scroll :: the background is *fixed relative to the element itself and does not scroll with its contents* (background *image will scroll with the page*). It is effectively attached to the element's border.
...
*w3schools*: defines how far the background (color, image, or gradient) should extend within an element.
...
*Values*:
- border-box :: the background extends to the outside edge of the border (background extends behind the border) (but underneath the border in z-ordering).
- padding-box :: the background extends to the outside edge of the padding (background extends to the inside edge of the border). No background is drawn beneath the border.
- content-box :: the background is painted within (clipped to) the content box (background extends to the edge of the content box).
- text :: the background is painted within (clipped to) the foreground text.
- border-area :: the background is painted within (clipped to) the area painted by the border, taking ~border-width~ and ~border-style~ into account but ignoring any transparency introduced by ~border-color~.
...
#+begin_src css
  ...
  /* Keyword values */
  background-clip: border-box | padding-box | content-box | text | border-area;
...
*w3schools*: specifies the origin position (the background positioning area) of a background image.
...
*Values*:
- border-box :: the background is positioned *relative to the border box* (background image starts from the upper left corner of the border)
- padding-box :: the background is positioned *relative to the padding box* (background image starts from the upper left corner of the padding edge)
- content-box :: the background is positioned *relative to the content box* (background image starts from the upper left corner of the content)
...
#+begin_src css
  ...
  /* Let the image start from the upper left corner of the content: */
  #example1 {
...
#+begin_src css
  ...
  background-color: hsl(50 33% 25% / 75%); /* 75% opaque (25% transparent) */
...
*w3schools*: the ~background-blend-mode~ property defines the blending mode of each background layer (color and/or image).
...
#+begin_src css
  ...
  mix-blend-mode: screen | overlay | darken | lighten | color-dodge |
                  color-burn | hard-light | soft-light | difference |
                  exclusion | hue | saturation | color | luminosity |
                  plus-darker | plus-lighter;


  /* A container with a red background and an image that blends with */
  /* the red container (darken): */
  .container {
    background-color: red;
  }
...
The ~<color-interpolation-method>~ specifies what interpolation method should be used to mix the colors. It consists of the ~in~ keyword and the color space the colors should be mixed in. The color space must be one of the available color spaces listed in the formal syntax. Depending on the color space used, you can optionally direct the hue to be mixed along a longer or shorter path.

The ~<rectangular-color-space>~ category includes ~srgb~, ~srgb-linear~, ~display-p3~, ~a98-rgb~, ~prophoto-rgb~, ~rec2020~, ~lab~, ~oklab~, ~xyz~, ~xyz-d50~, and ~xyz-d65~.

The ~<polar-color-space>~ category includes ~hsl~, ~hwb~, ~lch~, and ~oklch~. With these you can optionally follow the color space name with a ~<hue-interpolation-method>~. This value defaults to ~shorter hue~, but can also be set to ~longer hue~, ~increasing hue~, or ~decreasing hue~.
...
- Repeating gradient :: repeating gradients duplicate a gradient as much as necessary to fill a given area. They are generated with the [[repeating-linear-gradient()]], [[repeating-radial-gradient()]], and [[repeating-conic-gradient()]] functions.

#+begin_src css

  /* Default: CSS linear-gradient() Function */
  background-image: linear-gradient(top to bottom, ...);
  /* Syntax: CSS linear-gradient() Function */
  background-image: linear-gradient(direction, color-stop1, color-stop2, ...);
  /* Syntax: Linear Gradient - Using Angles */
  background-image: linear-gradient(angle, color-stop1, color-stop2);


  /* A linear gradient. Direction - Left to Right */
  .linear-gradient {
  ...
  /* Repeating linear and radial gradient */
  .linear-repeat {
...
**** linear-gradient()
***** Brief
CSS function creates an image consisting of a *progressive transition between two or more colors* along a straight line. Its result is an object of the [[gradient]] data type, which is a special kind of ~<image>~.
...
#+begin_src css
  ...
  linear-gradient(45deg, blue 50px, red 300px)
  ...
  /* numbered with color = stop | numbered only = hint */
  /* Color stop: starting blue, turning green at 40% of its length,
         and finishing red */
  linear-gradient(0deg, blue, green 40%, red)

  /* Color hint: starting red, getting to the midpoint color
...
*Note (RF)*: a good way to visualize this is to use the Gradient tool in a box in *Inkscape*; this displays the /gradient line/ and you can add any number of /color-stop/ points.
...
#+begin_src css
  ...
  linear-gradient(red 0%, blue 50%, yellow 100%)

  linear-gradient(red,    blue,        white,        yellow);
  linear-gradient(red 0%, blue 33.33%, white 66.66%, yellow 100%);
...
#+begin_src css
  ...
  /* En 25% esta en su rojo puro y comienza a degradar de rojo a azul */
  /* En 50% llega degradando a azul puro y comienza a degradar de azul a negro */
  /* En 75% esta en su negro puro pero como no hay más color para degradar */
  /* continua mandando solo negro puro */
  0%|----25%|----50%|----75%|----100%
  +-----------------------------------------------------------+
  |                                                           |
  +-----------------------------------------------------------+
...
#+begin_src css
  ...
  /* ManzDev: primer cuadrante en rojo */
  background:
    conic-gradient(
      transparent 0,
      red 0 0.25turn, /* desde 0 hasta 0.25 de una vuelta */
      transparent 0.25turn 1turn
      /* mismo que */
      red 0deg, red 90deg,
      transparent 90deg
    );
  /* ManzDev: primer cuadrante degradado en rojo */
  background:
    conic-gradient(
      transparent 0,
      red 0.1turn 0.2turn, /* desde 0 hasta 0.25 de una vuelta */
      transparent 0.25turn 1turn
    );
...
Creates an image consisting of repeating linear gradients. It is similar to [[linear-gradient()]] and takes the same arguments, but it repeats the color stops infinitely in all directions so as to cover its entire container. The function's result is an object of the ~<gradient>~ data type, which is a special kind of ~<image>~.
...
#+begin_src css
  ...
  /* ManzDev: lineas verdes de 1px, espacio transparente de 24px */
  /*          genera serie de lineas horizantales de color verde */
...
#+begin_src css
  ...
  repeating-conic-gradient(from 0deg, red 0deg 30deg,
                           yellow 30deg 60deg,
                           blue 60deg 90deg);
...
- Optionally, the ~inset~ keyword. Changes the shadow from an outer box-shadow to an inner box-shadow (as if the content is pressed into the box).
...
#+begin_src css
  ...
  /* ManzDev: varias cajas */
  .box {
    width: 64px;
    heigth: 64px;
    background: white;
    box-shadow:
      /* una elemento (4% de la region visible de ancho y 50% de alto) */
      4vw 50vh 0 white,
      /* otra elemento */
      18vw 88vh 0 white,
  }
...
*w3schools*:
Allows you to change property values smoothly, over a given duration. To create a transition effect, you must specify the CSS property you want to add a transition to, and the duration of the transition.
...
So, from this code ~div {transition: width 2s;}~, the transition effect will start when the ~width~ property changes value.

Now, we add a ~div:hover~ class that specifies a new value for the ~width~ property when a user mouses over the ~<div>~ element: ~div:hover {width: 300px;}~.
...
*w3schools*: an animation lets an element gradually *change from one style to another*. You can change as many CSS properties you want, as many times as you want. To use CSS animation, you must specify some keyframes for the animation. Keyframes hold what styles the element will have at certain times.
...
#+begin_src css
  ...
  animation-name: none | sliding | -specific | "sliding-vertically";
  animation-duration: auto | 6s | 120ms ;
  animation-timing-function: ease | ease-in | ease-out | ease-in-out |
                      linear | step-start | step-end;
  animation-iteration-count: infinite | 3 | 2.4;
  animation-fill-mode: none | forwards | backwards | both;
  animation-delay: 3s | 0s | -1500ms ;
  animation-direction: normal | reverse | alternate | alternate-reverse ;
  animation-play-state: running | paused;
  animation-timeline: none | auto | --time_name | scroll() | view(inline);
...
- zero or one occurrences of the following values:
  - ~<single-easing-function>~ type of transition
  - ~<single-animation-iteration-count>~ times the animation is played
  - ~<single-animation-direction>~ direction in which the animation is played
  - ~<single-animation-fill-mode>~ how styles should be applied to the animation's target before and after its execution
  - ~<single-animation-play-state>~ whether the animation is playing or not

- an optional name for the animation, which may be ~none~, a ~<custom-ident>~, or a ~<string>~

*Note*: the recommended practice is to specify a value for animation-name as the last value in a list of values when using the ~animation~ shorthand
...
#+begin_src css
  ...
  /* ManzDev */
  /* nombre, duración, ritmo, número de veces delay*/
  animation: myAnimation 3s linear infinite 2s;
  /* name, duration, timing-function(type), iteration-count delay */
...
#+begin_src css
  ...
  @keyframes myAnimation {
    /* we have used the keywords "from" and "to" in the @keyframes rule, */
  ...
  /* ManzDev */
  @keyframes myAnimation {
    /* al principio de la animacion la altura sea 100% */
    0%   {height: 100%;}
    /* al final de la animacion la altura sea 0% */
    100% {height: 0%;}
  }
...
To use keyframes, create a ~@keyfram~ es rule with a name that is then used by the ~animation-name~ property to match an animation to its keyframe declaration. Each ~@keyframes~ rule contains a style list of keyframe selectors, which specify percentages along the animation when the keyframe occurs, and a block containing the styles for that keyframe.
...
*Keyframe selectors*
CSS keyframe selectors identify specific points in an animation timeline where keyframe styles should be applied. These selectors are used exclusively within the ~@keyframes~ at-rule.
...
#+begin_src css
  ...
  /* The opacity property adds transparency to the background of an */
  /* element, and to all of its child elements as well. This makes the */
  /* text inside a transparent element hard to read: */
  div.first {
    opacity: 0.1;
  }
  ...
  /* To not apply opacity to child elements (like in the example above) */
  /* use RGBA color values instead. The following example sets the */
...
*w3schools*: defines visual effects (like blur and saturation) to an element (often <img>).

Several functions, such as [[blur()]] and [[contrast()]], are available to help you achieve predefined effects. When this happens, the filters are applied in order.

The ~filter~ property is specified as ~none~ or one or more of the functions. If the parameter for any function is invalid, the function returns ~none~. Except where noted, the functions that take a value expressed with a percent sign (as in ~34%~) also accept the value expressed as decimal (as in ~0.34~).
...
#+begin_src css
  ...
  filter: blur(5px) | brightness(0.4) | contrast(200%) |
          drop-shadow(16px 16px 20px blue) | grayscale(50%) |
          hue-rotate(90deg) | invert(75%) | opacity(25%) |
          saturate(30%) | sepia(60%);
...
**** aspect-ratio
Allows you to define the desired *width-to-height ratio* of an element's box. This means that even if the parent container or viewport size changes, the browser will adjust the element's dimensions to maintain the specified width-to-height ratio. The specified aspect ratio is used in the calculation of auto sizes and some other layout functions.
...
#+begin_src css
  ...
  /* Syntax */
  aspect-ratio: number(width) / number(height) | initial | inherit;
  aspect-ratio: 1 / 1;
  aspect-ratio: 1;
...
** The Box model
*** Box model
In CSS we have several types of boxes that generally fit into the categories of block boxes and inline boxes. The *type* refers to how the box behaves in *terms of page flow* and in *relation to other boxes* on the page. Boxes have an [[Inner display type (flow layout, grid, flex)][inner display type]] and an [[Outer display type (block, inline, inline-block)][outer display type]].
...
#+begin_src artist
  ...
  |   |   |   |              content area               |   |   |   |
  |   |   |   +-----------------------------------------+   |   |   |
  |   |   | padding area (push the content away from border)|   |   |
  |   |   +-------------------------------------------------+   |   |
  |   |        border                                           |   |
  |   +---------------------------------------------------------+   |
  |        margin (pushes other elements away from the box)         |
  +-----------------------------------------------------------------+
...
*** Containing block
**** Containing block
The *size* and [[position]] of an element are often impacted by its containing block. Most often, the containing block is the *content area of an element's nearest containing block-level ancestor*, but this is [[Identifying the containing block][not always the case]].
...
*Note*: the *containing block* is the nearest containing block-level of the element (obviously the element must be nested inside the containing) and the containing block has a ~position~ value other than ~static~ ( ~fixed~ , ~absolute~ , ~relative~ , or ~sticky~ ), if there are no other positioned containing ancestors, then ~<html>~ will be the containing block. (INS)
...
**** Summary: percentage value, position relative, absolute
This table serves as a guide; the actual, detailed definitions are above. (INS)

|             | depends on | edge of | container has       |
|-------------+------------+---------+---------------------|
| percentages | container  |         |                     |
| % static    | container  |         |                     |
| % relative  | container  | content | block, flex, grid   |
| % absolute  | container  | padding | non-static property |

*Note*: if there are no other positioned containing ancestors, then ~<html>~ will be the containing block.
...
That is, in the ~width~ property the writing-mode is independent - always means left-to-right width, but in the ~inline-size~ property the writing-mode is dependent - changes meaning based on context.

Use ~width~ for fixed measurements and when you specifically want to set the width regardless of writing mode.
Use ~inline-size~ for adaptive layouts that respond to the writing direction, making it particularly useful for internationalization and responsive designs.
...
-------------------------------------------------------------------------
justify-content | main/inline | content within element | block containers
      1         |             |   (adjust padding)     | flex containers
align-content   | cross/block |                        | grid containers
-------------------------------------------------------------------------
justify-self    |    inline   |                        | block-level boxes
                |             |                        | absolutely-positioned
      2         |             | element within parent  | boxes, grid items
                |             |    (adjust margins)    |
align-self      | cross/block |                        | absolutely-positioned
                |             |                        | boxes, flex items,
                |             |                        | grid items
-------------------------------------------------------------------------
justify-items   |    inline   |                        | block containers
      3         |             |   items inside box     | grid containers
                |             | (controls child items  |
align-items     | cross/block |   justify-self:auto)   | flex containers
                |             |                        | grid containers
            1                         2                         3
  ┌────────────────────┐    +-------------------+    ┌─────────────────────┐
  │         ^          │    |         ^         |    │   ^      | ^        │
...
*** Alignment container
The alignment container is the rectangle that the [[Alignment subject][alignment subject]] is aligned within. This is defined by the layout mode; it is usually the alignment subject's containing block, and assumes the writing mode of the box establishing the containing block. An alignment container may contain one or many alignment subjects.

*** Alignment subject
In CSS box alignment the alignment subject is the thing (or things) being aligned within the [[Alignment container][alignment container]] by the property.

For [[justify-self]] and [[align-self]], the alignment subject is the *margin* box of the box the property is set on, using the writing mode of that box. For [[justify-content]] and [[align-content]], the writing mode of the box is also used.

For ~justify-self~ or ~align-self~, or when setting these values as a group with [[justify-items]] or [[align-items]], this will be the *margin* box of the element that this property is being used on. The ~justify-content~ and ~align-content~ properties differ per layout method.
...
- Flex containers :: For ~justify-content~, the flex items in each flex line. For ~align-content~, the flex lines. Note, this only has an effect on multi-line flex containers.
...
***** align-content and justify-content
In multi-column layout, the [[Alignment container][alignment container]] is the content box of the multicol container. The [[Alignment subject][alignment subject]] is the column box.
...
**** align-items
Sets the [[align-self]] value on all direct children as a group. In flexbox, it controls the alignment of items on the cross axis. In grid layout, it controls the alignment of items on the block axis within their grid areas.
...
The [[Position][position]] property *allows you to precisely control the placement of boxes inside other boxes*. ~static~ positioning is the default in normal flow.
...
*Note*: to stretch horizontal, make [[flex items][flex:1]] in the flex items.
...
**** Flex/Grid and absolutely positioned elements
...
Lines can be addressed using their line number. In a left-to-right language such as English, column line 1 will be on the left of the grid, row line 1 on the top. Lines numbers respect the writing mode of the document.
...
#+begin_src css
  ...
  /* Syntax */
  align-items: start | end | center | stretch | space-between
...
Estas propiedades funcionan exactamente igual que sus análogas ~justify-items~ ~align-items~ y tienen los mismos valores, sólo que en lugar de indicarse en el elemento padre contenedor, se hace sobre un elemento hijo y repercute en dicho elemento hijo específicamente.
...
***** Grid item that has a grid container as a positioning context
...
***** Grid item that has a grid container that is not a position context
...
***** Nested grid item that has a grid area as a positioning context
...
**** As many columns as will fit (like Flex) (~repeat()~)(~auto-fill~, ~auto-fit~)
...
*ManzDev*: ~auto-fill~ indica al navegador que *rellene* el contenedor grid con múltiples elementos hijos dependiendo del tamaño del viewport (región visible del navegador). ~auto-fit~ *ajusta* el tamaño de los ítems para que cubran el tamaño máximo de la fila. Si utilizamos ~repeat(auto-fill, minmax(300px, 1fr)~, el navegador se va a encargar de que los elementos hijos con el tamaño mínimo quepan en la primera fila, y los que no quepan, se desplacen a las siguientes filas del grid, consiguiendo un efecto similar al las [[Media Queries][media queries]].
...
Creating a grid to placing things on the grid. Grid always has lines — these are numbered beginning with 1 and relate to the writing mode of the document.
...
*Note*: You can also use the value -1 to target the end column or row line, then count inwards from the end using negative values. Note also that lines count always from the edges of the [[The difference between explicit and the implicit grid][explicit grid]], not the implicit grid.
...
- ~grid-area: auto~ coloca la celda en la próxima área vacía que se encuentre disponible
...
***** Alinear items hijos individualmente
...
**** Centrar verticalmente (~align-content~ en block)
...
**** ems and rems
The *em* unit means "my *parent element's font-size*" (in typography 'em' comes from the width of the uppercase letter 'M') in the case of typography. The [[li][<li>]] elements inside the [[ul][<ul>]] with a ~class~ of ~ems~ take their sizing from their parent. So each successive level of nesting gets progressively larger, as each has its font size set to ~1.3em~ — 1.3 times its parent's font size.
...
#+begin_src css
  ...
  @font-face {
    ...
    src: url('fonts/cicle_fina-webfont.woff2') format('woff2'),
         ...
         /* url('fonts/cicle_fina-webfont.ttf') format('truetype') ManzDev */
...
- Use WOFF2 and WOFF better compatibility and available in almost all browsers. The TTF format is sometimes not supported and takes up more space (manzDev).
...
To control inheritance, you can use the ~initial~, ~inherit~, ~unset~, ~revert~, or ~revert-layer~ properties defined by [[CSS-wide values]].

*** Cascade
At a very simple level, this means that the origin, the cascade layer, and the order of CSS rules matter. When two rules from the same cascade layer apply and both have equal [[Specificity][specificity]], the one that is defined last in the stylesheet is the one that will be used.
...
[[Media Queries][Media queries]] can help with Responsive Web Design, *but are not a requirement*. [[Flexbox][Flexible grids]], [[Relative length units][relative units]], and [[min- and max- sizes][minimum and maximum unit]] values *can be used* without queries.
...
#+begin_src html
  ...
  <a href="https://example.com/other/image.jpg">Link</a>
...
** How page request and browsers work
*** Overview
...
4. [[Example website logical sections in style sheet]]
5. [[Example website mobile first responsive design]]
...
Make sure that your site is secure, fast, accessible to all, and works on all devices.

**** Keywords and placement locations
...
**** Check your links
[[Googlebot]] navigates from URL to URL by fetching and parsing links, [[Sitemaps][sitemaps]], and redirects. Googlebot treats every URL as if it's the first and only URL it has seen from your site.
...
*** Googlebot can find all the URLs
