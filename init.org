...
#+begin_src emacs-lisp
  ...
  ;; Caution, these measurements aren't exact; they were
  ;; determined through trial and error:
  ;; top left
  (add-to-list 'default-frame-alist '(left . 1356))
  (add-to-list 'default-frame-alist '(top . 30))
  ;; top right
  ;; (add-to-list 'default-frame-alist '(left . 1926))
  ;; (add-to-list 'default-frame-alist '(top . 30))
  ;; bottom right
  ;; (add-to-list 'default-frame-alist '(left . 1926))
  ;; (add-to-list 'default-frame-alist '(top . 419))
  ;; bottom left
  ;; (add-to-list 'default-frame-alist '(left . 1356))
  ;; (add-to-list 'default-frame-alist '(top . 419))

  (defun rf-position-and-resize-frame (&optional prefix-arg)
    "Position and resize the frame.
  The size is adjusted to fit the visible part of the laptop;
  the main positioning is in the top left corner of the monitor.
  With the universal-command, move to other corner."
    (interactive "p")
    (set-frame-size (selected-frame) 168 39)
    (cond
     ((eq prefix-arg 1) ;; top left
      (set-frame-position (selected-frame) 1356 30))
     ((eq prefix-arg 2) ;; top right
      (set-frame-position (selected-frame) 1926 30))
     ((eq prefix-arg 3) ;; bottom right
      (set-frame-position (selected-frame) 1926 419))
     ((eq prefix-arg 4) ;; bottom left
      (set-frame-position (selected-frame) 1356 419))))

  (defun rf-position-and-resize-frame-top-right ()
    "Position and resize the frame in the top right."
    (interactive)
    (set-frame-size (selected-frame) 168 39)
    (set-frame-position (selected-frame) 1926 30))

  (defun rf-position-and-resize-frame-bottom-right ()
    "Position and resize the frame in the bottom right."
    (interactive)
    (set-frame-size (selected-frame) 168 39)
    (set-frame-position (selected-frame) 1926 419))

  (defun rf-position-and-resize-frame-bottom-left ()
    "Position and resize the frame in the bottom left."
    (interactive)
    (set-frame-size (selected-frame) 168 39)
    (set-frame-position (selected-frame) 1356 419))

  (defun rf-move-frame-to-laptop ()
    "Move the frame to the laptop."
    (interactive)
    (set-frame-size (selected-frame) 168 39)
    (set-frame-position (selected-frame) 60 512))
  ...
  (defun rf-make-frame-command-mod ()
   ...
   When called from Lisp, returns the new frame."
    (interactive)
    (if (display-graphic-p)
        ;; Trick by subtracting 1, otherwise a tiny frame will be created.
        ;; When the width is 168, the frame is created tiny (IDKW).
...
#+begin_src emacs-lisp
  ...
  (use-package goto-chg
    ...
    :bind (:repeat-map goto-last-repeat-map
                       ("d" . goto-last-change)
                       ("b" . goto-last-change-reverse)))
...
#+begin_src emacs-lisp
  ...
  (keymap-set minibuffer-local-map "M-s" nil)
  (keymap-set minibuffer-local-map "C-," 'se-take-the-word-at-point)
  ;; Set next/previous history in case the vertico-map is unavailable.
  (keymap-set minibuffer-local-map "C-S-h" 'previous-line-or-history-element)
  (keymap-set minibuffer-local-map "C-S-n" 'next-line-or-history-element)
  ;; Set vertico-first and vertico-last
  (keymap-set minibuffer-local-map "M-s" 'vertico-first)
  (keymap-set minibuffer-local-map "M-S" 'vertico-last)
  ;; (keymap-set minibuffer-local-map "C-S-h" 'previous-history-element)
  ;; (keymap-set minibuffer-local-map "C-S-n" 'next-history-element)
...
#+begin_src emacs-lisp
  ...
  (defalias 'rf-fold-org-english-headers
  ...
  (defalias 'rf-swap-org-header-with-text
    (kmacro "C-S-<home> C-SPC C-S-<end> S-<delete> C-<up> C-<right> C-<left> S-<insert> <return> S-<end> <delete>"))

  ;; Run the last interactive command make performed by pressing 'M-x'
  (defalias 'rf-last-interactive-command
    (kmacro "<menu> <return>"))
...
#+begin_src emacs-lisp
  ...
  (use-package expand-region
    :bind (:repeat-map extend-selection-repeat-map
                       (")" . rf-expand-region))
    :config
  ...
  ;; To be used in the `rf-beginning-of-line' command.
  (defun rf-bovlp ()
  ...
  ;; To be used in the `rf-end-of-line' command.
  (defun rf-eovlp ()
  ...
  (defun rf-pos-bovl (&optional N)
    "Return the position of the first character on the current visual line.
  ...
  (defun rf-pos-eovl (&optional N)
    "Return the position of the last character on the current visual line.
  With optional argument N non-nil, move forward N - 1 lines first."
    (interactive)
    (save-excursion
      (when N
  ...
  ;; Make the `beginning-of-line' command smart.
  (defun rf-beginning-of-line (arg)
    "Move point to beginning of line or end of line.
  If the point is at the beginning of line, then move it to end of the line."
    (interactive "p")
  ...
  ;; Make the `end-of-line' command smart.
  (defun rf-end-of-line (arg)
    "Move point to end of line or beginning of line.
  If the point is at the end of line, then move it to beginning of the line."
    (interactive "p")
    (if (bound-and-true-p visual-line-mode)
        (if (rf-eovlp)
            (beginning-of-visual-line)
  ...
  ;; Modified version of Xah to also cut the visual line.
  (defun xah-cut-line-or-region-mod ()
    "Cut current line/(visual line) or selection.
   If `universal-argument' is called first, cut whole buffer
   ...
   Version: 2015-06-10"
    ...
    (if current-prefix-arg
        (progn ; not using kill-region
          (kill-new (buffer-string))
          (delete-region (point-min) (point-max)))
      (progn (if (region-active-p)
                 (kill-region (region-beginning) (region-end) t)
               ;; Modified here to cut the visual line.
               (if (bound-and-true-p visual-line-mode)
                   (kill-region (rf-pos-bovl) (rf-pos-bovl 1))
                 (kill-region
                  (line-beginning-position)
                  (line-beginning-position 2)))))))

  ;; Modified version of Xah to also work on visual lines.
  (defun xah-copy-line-or-region-mod ()
  ...
  If `universal-argument' is called first, copy whole buffer
  (respects `narrow-to-region').
    ...
    (cond
     ...
     ((eq last-command this-command)
      (if (eobp)
        ...
        (progn
          (kill-append "\n" nil)
          ;; Modified here to also work on visual lines.
          (if (bound-and-true-p visual-line-mode)
              (progn
                (kill-append (buffer-substring (rf-pos-bovl)
                                               (rf-pos-eovl)) nil)
                (end-of-visual-line))
            (progn
              (kill-append (buffer-substring (line-beginning-position)
     ...
     ((eobp)
      (if (eq (char-before) 10)
        ...
        (if (bound-and-true-p visual-line-mode)
            (progn
              (copy-region-as-kill (rf-pos-bovl) (rf-pos-eovl))
              (end-of-visual-line))
          (progn
  ...
  ;; Modified version to also work on visual lines and repeated maps.
  (defun rf-backward-kill-line (&optional arg)
    "Kill back from point to start of line/(visual line)."
    (interactive)
    (if (bound-and-true-p visual-line-mode)
        (progn
          (when (rf-bovlp)
            (backward-char))
          (kill-region (point) (rf-pos-bovl)))
      (progn
        (when (bolp)
          (backward-char))
  ...
  ;; Function that replaces 'eval-region' and 'eval-last-sexp' in a command.
  (defun rf-eval-last-sexp-or-region (argument)
    "Wrapper that calls `eval-region' or `eval-last-sexp'.
  If there's a region, call `eval-region'; otherwise, call `eval-last-sexp'."
    (interactive "P")
    (if (use-region-p)
  ...
  (defun rf-jump-to-newline ()
    "Move cursor to end of line, and insert a newline.
  Useful in programming modes to jump to a newline without
  having to move to the end of the line first."
    (interactive)
    (move-end-of-line 1)
    (newline-and-indent))


  (defun rf-toggle-xah-narrow-to-region ()
    "Wrapper of `xah-narrow-to-region' to toggle between narrow and widen.
  Same as `narrow-to-region', but if no selection, narrow to
  the current block.
  When it's in `org-mode' and no region run `org-toggle-narrow-to-subtree'."
    (interactive)
    (let (xbeg xend)
      ...
      (if (buffer-narrowed-p)
        ...
        (if (and (eq major-mode 'org-mode) (not (region-active-p)))
            (org-narrow-to-subtree)
          (narrow-to-region xbeg xend)
          (message "Buffer narrowed to current subtree")))))


  (defun rf-xah-select-block ()
    "Wrapper of `xah-select-block' that avoids aditional blankline."
    (interactive)
    (xah-select-block)
    (when (not (eobp))
      (backward-char)))
  ...
  (defun rf-camelcase-to-kebabcase (string)
    "Convert the camelCase string to kebab-case."
    (interactive)
...
#+begin_src emacs-lisp
  ...
  (use-package tempel
    ...
    :config
    (setq tempel-path "/home/rich/.emacs.d/etc/tempel/*.eld")
    ;; Add (i template) to include templates by name in another template.
    ...
    ;; Define a custom element * to repeat a template a number of times
    ...
    ;; Reload modified templates quickly
    (defun rf-global-tempel-reload ()
    "Turn the Temple off and on to enable all new templates."
      (interactive)
      (basic-save-buffer)
  ...
  ;;; Try later
  ;; This will take the LSP template provided by the server and convert
  ;; it into a format tempel can take advantage of.
...
#+begin_src emacs-lisp
  ...
  (use-package yasnippet
    :bind (:map yas-minor-mode-map
                ...
                ("<tab>" . yas-next-field-or-maybe-expand)
                ("C-<iso-lefttab>" . yas-next-field-or-maybe-expand))
    :config
    (yas-reload-all))
...
#+begin_src emacs-lisp
  ...
  (keymap-set occur-mode-map "c" 'previous-error-no-select)
  (keymap-set occur-mode-map "t" 'next-error-no-select)
  (keymap-set occur-mode-map "C-S-c" 'occur-prev)
  (keymap-set occur-mode-map "C-S-t" 'occur-next)
  (keymap-set occur-mode-map "b" 'clone-buffer)
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'cua-rect
    (keymap-set cua--rectangle-keymap "C-S-c" 'cua-resize-rectangle-up)
    (keymap-set cua--rectangle-keymap "C-S-t" 'cua-resize-rectangle-down)
    (keymap-set cua--rectangle-keymap "C-S-h" 'cua-resize-rectangle-left)
    (keymap-set cua--rectangle-keymap "C-S-n" 'cua-resize-rectangle-right)
    (keymap-set cua--rectangle-keymap "C-S-d" 'cua-resize-rectangle-bol)
    (keymap-set cua--rectangle-keymap "C-S-b" 'cua-resize-rectangle-eol))
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'completion-preview
    ...
    (keymap-set completion-preview-active-mode-map "M-i" nil)
    (keymap-set completion-preview-active-mode-map "M-l"
...
#+begin_src emacs-lisp
  ...
  (use-package drag-stuff
    :bind (("M-<up>" . drag-stuff-up)
           ...
           :repeat-map drag-stuff-repeat-map
           ("c" . drag-stuff-up)
           ("t" . drag-stuff-down))
    :config
  ...
  ;; To be used in `package-menu-mode-map'
  (with-eval-after-load 'tabulated-list
    (keymap-set tabulated-list-mode-map "c" 'previous-line)
    (keymap-set tabulated-list-mode-map "t" 'next-line))
...
#+begin_src emacs-lisp
  ...
  (defun rf-recently-not-visible-buffer ()
    "Display most recently not visible buffer, in the selected window."
    (interactive)
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'ibuffer
    ...
    (keymap-set ibuffer-mode-map "c" 'ibuffer-backward-line)
    (keymap-set ibuffer-mode-map "t" 'ibuffer-forward-line)
    (keymap-set ibuffer-mode-map "p" 'ibuffer-toggle-marks)
    (keymap-set ibuffer-mode-map "o" 'ibuffer-visit-buffer-other-window-noselect)
    (keymap-set ibuffer-mode-map "C-o" 'ibuffer-visit-buffer-other-window)
...
#+begin_src emacs-lisp
  ...
  (use-package orderless
    :config
    ;; Orderless style dispatchers (Ensure that the $ regexp works
    ;; with consult-buffer, and other things)
...
#+begin_src emacs-lisp
  ...
  (use-package consult
    ...
    (("<kp-equal>" . consult-buffer)
     ("M-<kp-equal>" . consult-buffer-other-window)
     ("C-n" . consult-buffer)
    ...
    ;; Modifying `consult-fd/find' to search starting in the home/root dir
    (defun rf-consult-fd (&optional prefix-arg)
      "Search for files with `fd' in home directory.
  With the universal-command, starting in the root directory."
      (interactive "p")
      (if (eq prefix-arg 4)
          (consult-fd "/")
        (consult-fd "~/")))
    (defun rf-consult-find (&optional prefix-arg)
      "Search for files with `find' in home directory.
    With the universal-command, starting in the root directory."
      (interactive "p")
      (if (eq prefix-arg 4)
          (consult-find "/")
...
#+begin_src emacs-lisp
  ...
  (use-package corfu
    ...
    ;; Remove UI when there's no match
    (corfu-quit-no-match t)  ;; or 'separator
    ...
    ;; Enable movements bindings in `corfu-popupinfo-mode'
    (with-eval-after-load 'corfu-popupinfo
      (keymap-set corfu-popupinfo-map "M-c" #'corfu-previous)
      (keymap-set corfu-popupinfo-map "M-t" #'corfu-next))
    ;; Completing in the Eshell
    (add-hook 'eshell-mode-hook
              (lambda () (setq-local corfu-quit-at-boundary 'separator
                                     corfu-preselect 'first
    ...
    ;; Activate corfu-auto and reload corfu-mode from the interactive
  ...
  ;; If the server does not guarantee returning all valid completions
  ;; matching the prefix, you will miss potentially valid completions.
  ;; Depending on if Eglot returns all or sufficiently many candidates
  ;; in the first place you may or may not need the cape-wrap-buster.
...
#+begin_src emacs-lisp
  ...
  (use-package cape
    ...
    ;; Cape working to combine emacs-lisp and dabbrev completion candidates.
    ;; Also adds cape-file to the list of completion at point functions,
    ;; to try as a “back-up”, e.g. in strings and comments.
    ;; Put a minimum length of 5 characters to avoid small word pollution.
    (defun mi-ignore-elisp-keywords (cand)
      (or (not (keywordp cand))
          (eq (char-after (car completion-in-region--data)) ?:)))
    (defun mi-setup-elisp ()
    ...
    (add-hook 'emacs-lisp-mode-hook #'mi-setup-elisp))
  ;; ;; Making a Cape Super Capf for Eglot:
  ;; ;; Use Cape’s cape-capf-super function to compose our own completion
  ;; ;; at point using eglot’s eglot-completion-at-point, tempel’s
  ;; ;; tempel-expand and Cape’s cape-file
  ;; (defun mi-eglot-capf ()
  ;;   (setq-local completion-at-point-functions
  ;;               (list (cape-capf-super
  ;;                      #'eglot-completion-at-point
  ;;                      #'cape-file ; or maybe in the first place
  ;;                      #'tempel-expand))))
  ;; (add-hook 'eglot-managed-mode-hook #'mi-eglot-capf)
    ...
    (defun re-embark-kill-buffer (&optional arg)
      ...
      (if-let ((targets (embark--targets)))
          (let* ((target
                 ...
                 (buffer (plist-get target :target)))
            (cond
             ;; MODIFICATION: don't delete the scratch or Messages buffer
             ((or (string= buffer "*scratch*")
                  (string= buffer "*Messages*")
                  (string= buffer "*eshell*"))
              (message "Try removing the buffer: %s." buffer))
             ((eq type 'buffer)
...
#+begin_src emacs-lisp
  ...
  ;;         vertico-prescient-override-sorting t) ;; or nil
  ;;   ;; This has to be here after `vertico',`vertico-prescient' for it to work.
  ;;   (with-eval-after-load 'vertico
  ;;     ;; Set another keymap to `prescient-toggle-map'
  ;;     (keymap-set vertico-map "M-l" prescient-toggle-map))
...
#+begin_src emacs-lisp
    ...
    (defvar-keymap delete-current-text-block-repeat-map
      :doc "Keymap for repeating `xah-delete-current-text-block' command."
      :repeat t
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap switch-resize-windows-repeat-map
    ...
    "b" #'balance-windows
    ;; Position frames.
    "u" #'rf-position-and-resize-frame
    "i" #'rf-position-and-resize-frame-top-right
    "x" #'rf-position-and-resize-frame-bottom-right
    "k" #'rf-position-and-resize-frame-bottom-left
    ;; Move the frame to the laptop.
    "e" #'rf-move-frame-to-laptop)
  (keymap-set global-map "M-U" switch-resize-windows-repeat-map)
  ...
  (defvar-keymap backward-kill-line-repeat-map
    :doc "Keymap to repeat commands to backward kill line."
    :repeat t
    "k" #'rf-backward-kill-line
    "j" #'xah-shrink-whitespaces)
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap newline-repeat-map
    :doc "Keymap to repeat newline command."
    :repeat t
    "m" #'newline)

  (defvar-keymap rf-jump-to-newline-repeat-map
    :doc "Keymap to repeat `rf-jump-to-newline' command."
    :repeat t
    "j" #'rf-jump-to-newline
    "o" #'open-line)
  ...
  (defvar-keymap previous-next-buffer-repeat-map
    :doc "Keymap to repeat previous and next buffer."
    :repeat t
    "<prior>" #'previous-buffer
    "<next>"  #'next-buffer)

  (defvar-keymap switch-buffer-repeat-map
    :doc "Keymap to repeat switch buffer and visible buffer commands."
    :repeat t
    "t" #'switch-to-previous-buffer
    "n" #'rf-recently-not-visible-buffer)
  ...
  (defvar-keymap mark-next-previous-repeat-map
    :doc "Keymap to repeat previous and next mark."
    :repeat t
...
#+begin_src emacs-lisp
  ...
  (dolist (binds
           '(
             ...
             ("M-b" . xah-forward-right-bracket)
             ("M-m" . xah-beginning-of-line-or-block)
             ("M-w" . xah-end-of-line-or-block)
...
#+begin_src emacs-lisp
  ...
  (dolist (binds
           '(
             ...
             ;; ("M-O" . )
             ;; ("M-E" . ~ace-window)
             ;; ("M-U" . +switch-resize-windows-repeat-map)
             ;; ("M-I" . )
             ;; ("M-D" . )
             ;; ("M-H" . )
             ;; ("M-T" . ~drag-stuff-down)
             ...
             ;; ("M-B" . )
             ("M-M" . scroll-down-command)
             ("M-W" . scroll-up-command)
             ;; ("M-V" . )
             ;; ("M-Z" . )
...
**** C-M-S-
Manage global =Meta= + =Control= + =Shift= key combinations. Generally intended to be used with keyboard macros so they don't interfere with other manual bindings.
...
#+begin_src emacs-lisp
  ...
  (dolist (binds
           '(
             ...
             ;; ("C-:" . )
             ;; ("C-<" . )
             ("C->" . rf-toggle-xah-narrow-to-region)
             ;; ("C-S-p" . )
             ;; ("C-S-y" . )
             ;; ("C-S-f" . ~consult-line-multi)
             ;; ("C-S-g" . )
             ...
             ;; ("C-S-x" . )
             ;; ("C-S-b" . )
             ("C-S-m" . scroll-other-window-down)
             ("C-S-w" . scroll-other-window)
             ;; ("C-S-v" . ~consult-yank-from-kill-ring)
             ;; ("C-S-z" . )
             ;; ("C-S-SPC" . )
             ))
...
#+begin_src emacs-lisp
  ...
  (dolist (binds
           '(
             ...
             ("<kp-4>" . rf-find-or-switch-to-tertiary-file)
             ("<kp-5>" . rf-find-or-switch-to-secondary-file)
             ("<kp-6>" . rf-find-or-switch-to-primary-file)

             ("<kp-1>" . rf-find-or-switch-to-javascript)
             ("<kp-2>" . eshell)
             ("<kp-3>" . scratch-buffer)
  ...
  (dolist (binds
           '(
             ...
             ;; ("S-<kp-7>" . ~mc/mark-previous-like-this)
             ("S-<kp-8>" . rf-find-or-switch-to-english)
             ("S-<kp-9>" . kmacro-start-macro)
             ;; ("S-<kp-add>" . )

             ("S-<kp-4>" . rf-find-or-switch-to-tertiary-test-file)
             ("S-<kp-5>" . rf-find-or-switch-to-secondary-test-file)
             ("S-<kp-6>" . rf-find-or-switch-to-primary-test-file)

             ("S-<kp-1>" . rf-find-or-switch-to-html-css-seo)
             ;; ("S-<kp-2>" . #Switch to application 4 (terminal))
             ("S-<kp-3>" . rf-find-or-switch-to-todo)
  ...
  (dolist (binds
           '(
             ...
             ;; ("M-<kp-7>" . )
             ("M-<kp-8>" . rf-find-or-switch-to-layout-maps)
             ;; ("M-<kp-9>" . )
             ;; ("M-<kp-add>" . )

             ("M-<kp-4>" . rf-find-or-switch-to-tertiary-aux-file)
             ("M-<kp-5>" . rf-find-or-switch-to-secondary-aux-file)
             ("M-<kp-6>" . rf-find-or-switch-to-primary-aux-file)

             ("M-<kp-1>" . rf-find-or-switch-to-emacs-elisp)
             ("M-<kp-2>" . rf-find-or-switch-to-shell)
             ("M-<kp-3>" . rf-find-or-switch-to-init)
  ...
  (dolist (binds
           '(
             ("C-<kp-equal>" . find-file-other-window)
             ;; ("C-<kp-divide>" . )
             ;; ("C-<kp-multiply>" . )
             ;; ("C-<kp-subtract>" . )
...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "<remap> <kill-region>" 'xah-cut-line-or-region-mod)
  (keymap-global-set "<remap> <kill-ring-save>" 'xah-copy-line-or-region-mod)
  (keymap-global-set "<remap> <yank>" 'xah-paste-or-paste-previous)
  (keymap-global-set "<remap> <yank-pop>" 'consult-yank-pop)

  ;; To be able to use `kill-ring-save' as a copy in CUA rec mode
  (with-eval-after-load 'cua-rect
    (keymap-set cua--rectangle-keymap "<remap> <xah-copy-line-or-region-mod>"
                'kill-ring-save))
...
#+begin_src emacs-lisp
  ...
  (use-package dired
    ...
    :bind (:map dired-mode-map
                ...
                ;; Re-enable in dired
                ("M-s" . ergoemacs-beginning-or-end-of-buffer)
                ;; Change prefix (M-s -> M-d)
                ("M-d d" . dired-do-isearch)
                ("M-d D" . dired-do-isearch-regexp)
                ("M-d f" . dired-isearch-filenames)
                ("M-d F" . dired-isearch-filenames-regexp)
                ;; To overwrite the default mode binding
                ("C-t" . switch-to-previous-buffer)

                ("_" . xah-dired-rename-space-to-underscore)
                ("-" . xah-dired-rename-space-to-hyphen)
                ;; thumbnail manipulation (image-dired)
                ;; It had the prefix "C-t" and that conflicted with the main
  ...
  ;; Dired to browse your images as thumbnails.
  (with-eval-after-load 'image-dired
    (keymap-set image-dired-thumbnail-mode-map "c"
                'image-dired-previous-line)
    (keymap-set image-dired-thumbnail-mode-map "t"
                'image-dired-next-line)
    (keymap-set image-dired-thumbnail-mode-map "h"
                'image-dired-backward-image)
    (keymap-set image-dired-thumbnail-mode-map "n"
                'image-dired-forward-image)
    (keymap-set image-dired-thumbnail-mode-map "e"
                'image-dired-thumbnail-display-external)
    (keymap-set image-dired-thumbnail-mode-map "H"
                'describe-mode)
    (keymap-set image-dired-thumbnail-mode-map "o"
                'image-dired-comment-thumbnail)
    (keymap-set image-dired-thumbnail-mode-map "a r"
                'image-dired-tag-thumbnail-remove)
    (keymap-set image-dired-thumbnail-mode-map "a t"
                'image-dired-tag-thumbnail)
    (keymap-set image-dired-thumbnail-mode-map "<mouse-1>"
                'image-dired-mouse-display-image)
    (keymap-set image-dired-thumbnail-mode-map "<mouse-2>"
...
#+begin_src emacs-lisp
  ...
  ;; Directory whose contents we trust
  ;; Useful for allowing `flymake-mode' in .el files
  (setq trusted-content '("~/.trusted/"))


  (defvar rf-primary-file nil
    "Primary file to switch to.
    This is one of the most common files, depending on what
    you're working on at the time. Its purpose is to add a
    shortcut to quickly switch to it.")

  (defvar rf-secondary-file nil
    "Secondary file to switch to.")

  (defvar rf-tertiary-file nil
    "Tertiary file to switch to.")

  (defvar rf-main-directory "/home/rich/projects/practice/solitaire/"
    "Main directory to be used with the main files.
  It's the initial path to the main files, such as `rf-main-file',
  `rf-secondary-file', and `rf-tertiary-file'.")

  (defvar rf-primary-test-file nil
    "Primary test file to switch to.
  Primary file of the test files set.
  Its purpose is to add a shortcut to quickly switch to it.")

  (defvar rf-secondary-test-file nil
    "Secondary test file to switch to.")

  (defvar rf-tertiary-test-file nil
    "Tertiary test file to switch to.")

  (defvar rf-main-test-directory "/home/rich/projects/practice/TEST/"
    "Main test directory to be used with the test main files.
  It's the initial path to the main test files, such as
  `rf-main-test-file', `rf-secondary-test-file',and `rf-tertiary-test-file'.")

  (defun rf-set-path-to-main-files (rf-main-directory)
    "Set the all path of the main files."
    (interactive "f")
    (setq rf-primary-file (expand-file-name "index.js" rf-main-directory)
          rf-secondary-file (expand-file-name "index.css" rf-main-directory)
          rf-tertiary-file (expand-file-name "index.html" rf-main-directory)))

  (rf-set-path-to-main-files rf-main-directory)

  (defun rf-set-path-to-test-files (rf-main-test-directory)
    "Set the all path of the main files."
    (interactive "f")
    (setq rf-primary-test-file
          (expand-file-name "script.js" rf-main-test-directory)
          rf-secondary-test-file
          (expand-file-name "styles.css" rf-main-test-directory)
          rf-tertiary-test-file
          (expand-file-name "home.html" rf-main-test-directory)))

  (rf-set-path-to-test-files rf-main-test-directory)

  (defvar rf-primary-aux-file nil
    "Primary auxiliary file to switch to.
    Primary file of the second most common set of files.
    Its purpose is to add a shortcut to quickly switch to it.")

  (defvar rf-secondary-aux-file nil
    "Secondary auxiliary file to switch to.")

  (defvar rf-tertiary-aux-file nil
    "Tertiary auxiliary file to switch to.")

  (defun rf-set-path-to-primary-aux-file (path-primary-aux-file)
    "Set the all path of the primary auxiliary file."
    (interactive "f")
    (setq rf-primary-aux-file path-primary-aux-file))

  (defun rf-set-path-to-secondary-aux-file (path-secondary-aux-file)
    "Set the all path of the secondary auxiliary file."
    (interactive "f")
    (setq rf-secondary-aux-file path-secondary-aux-file))

  (defun rf-set-path-to-tertiary-aux-file (path-tertiary-aux-file)
    "Set the all path of the tertiary auxiliary file."
    (interactive "f")
    (setq rf-tertiary-aux-file path-tertiary-aux-file))
  ...
  (defun rf-find-or-switch-to-file (file-path)
    ...
    (if file-path
      ...
      (message "There is no path to the file.")))
  ...
  (defun rf-find-or-switch-to-primary-file ()
    "Find or switch to the `rf-primary-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-primary-file))

  (defun rf-find-or-switch-to-secondary-file ()
    "Find or switch to the `rf-secondary-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-secondary-file))

  (defun rf-find-or-switch-to-tertiary-file ()
    "Find or switch to the `rf-tertiary-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-tertiary-file))

  (defun rf-find-or-switch-to-primary-test-file ()
    "Find or switch to the `rf-primary-test-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-primary-test-file))

  (defun rf-find-or-switch-to-secondary-test-file ()
    "Find or switch to the `rf-secondary-test-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-secondary-test-file))

  (defun rf-find-or-switch-to-tertiary-test-file ()
    "Find or switch to the `rf-tertiary-test-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-tertiary-test-file))

  (defun rf-find-or-switch-to-primary-aux-file ()
    "Find or switch to the `rf-primary-aux-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-primary-aux-file))

  (defun rf-find-or-switch-to-secondary-aux-file ()
    "Find or switch to the `rf-secondary-aux-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-secondary-aux-file))

  (defun rf-find-or-switch-to-tertiary-aux-file ()
    "Find or switch to the `rf-tertiary-aux-file' file."
    (interactive)
    (rf-find-or-switch-to-file rf-tertiary-aux-file))
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'tar-mode
    (keymap-set tar-mode-map "c" 'tar-previous-line)
    (keymap-set tar-mode-map "t" 'tar-next-line))
...
#+begin_src emacs-lisp
  ...
  (defun rf-org-bindings-setup ()
    ...
    ;; Trick to use TAB -> org-cycle only in org-mode
    (defvar-keymap org-char-word-movement-repeat-map
      :doc "Keymap for repeating character and word movement in org-mode."
      :parent char-word-movement-repeat-map
      :repeat t
      "TAB" #'org-cycle)
...
#+begin_src emacs-lisp
  ...
  (defun rf-org-properties-setup ()
    ...
    (setq org-structure-template-alist '(("a" . "src artist \n")
                                         ("c" . "src css \n")
                                         ("h" . "src html \n")
                                         ("j" . "src javascript \n")
                                         ("l" . "src emacs-lisp \n")
...
#+begin_src emacs-lisp
  ...
  (defun rf-org-editing-setup ()
    ...
    (defun rf-org-turn-word-link ()
      "Turn word/symbol or region into link."
      (interactive)
      (let (beg end bounds)
        (if (use-region-p)
...
#+begin_src emacs-lisp
  ...
  (use-package impatient-mode
    ...
    (setq imp-default-user-filters
          '((mhtml-mode) (html-ts-mode) (html-mode) (web-mode))))
  ...
  (use-package treesit
    ...
    (defun mp-setup-install-grammars ()
      ...
      (dolist (grammar
               ...
               ;; are known to work with Combobulate (20.) *and* Emacs.
               ;; The newer versions work well with this and template-literals
    ...
    (use-package combobulate
      ...
      :preface
      (defvar-keymap combobulate-cursor-map
        :doc "Keymap for combobulate-cursor commands."
        :prefix 'combobulate-cursor-map
        "h" #'combobulate-cursor-edit-node-by-text-dwim
        "t" #'combobulate-cursor-edit-node-type-dwim
        "n" #'combobulate-cursor-edit-node-siblings-dwim
        "s" #'combobulate-cursor-edit-sequence-dwim
        "g" #'combobulate-cursor-edit-query)
      (keymap-set combobulate-key-map "<f6> o" combobulate-cursor-map)

      (defvar-keymap combobulate-envelope-map
        :doc "Keymap for combobulate-envelope commands."
        :prefix 'combobulate-envelope-map
        "h" #'combobulate-javascript-envelope-wrap-parentheses
        "t" #'combobulate-javascript-envelope-if-statement
        "n" #'combobulate-javascript-envelope-tag
        "s" #'combobulate-javascript-envelope-expression)
      (keymap-set combobulate-key-map "<f6> ," combobulate-envelope-map)

      ;; Repeatable commands:
      (defvar-keymap combobulate-navigate-repeat-map
        :doc "Keymap for repeating programming navigation movements."
        :repeat t
        "c" #'combobulate-navigate-sequence-previous
        "t" #'combobulate-navigate-sequence-next
        "h" #'combobulate-navigate-logical-previous
        "n" #'combobulate-navigate-logical-next
        "g" #'combobulate-navigate-up
        "r" #'combobulate-navigate-down
        "d" #'combobulate-navigate-previous
        "b" #'combobulate-navigate-next
        "m" #'combobulate-navigate-beginning-of-defun
        "w" #'combobulate-navigate-end-of-defun)

      (defvar-keymap combobulate-drag-repeat-map
        :doc "Keymap to repeat combobulate-drag commands."
        :repeat t
        "c" #'combobulate-drag-up
        "t" #'combobulate-drag-down)

      (defvar-keymap combobulate-kill-node-repeat-map
        :doc "Keymap to repeat command to kill nodes."
        :repeat t
      ...
      :bind (
             ...
             ("M-n" . nil) ("M-p" . nil) ("M-k" . nil) ("M-h" . nil)
             ("M-e" . nil) ("M-a" . nil) ("M-N" . nil) ("M-P" . nil)
             ;; Navigating
             ...
             ("<f6> d" . combobulate-navigate-previous)
             ("<f6> b" . combobulate-navigate-next)
             ("<f6> m" . combobulate-navigate-beginning-of-defun)
             ("<f6> w" . combobulate-navigate-end-of-defun)
             ;; Editing
             ...
             :map combobulate-cursor-key-map
             ("C-S-c" . combobulate-cursor-prev-field)
             ("C-S-t" . combobulate-cursor-next-field)
             :map combobulate-javascript-map
             ("=" . nil))
      :config
      ;; Remove these bindings that couse conflicts
      (add-hook 'combobulate-javascript-minor-mode-hook
                (lambda () (keymap-unset combobulate-javascript-map "M-(")))
      (add-hook 'combobulate-css-minor-mode-hook
                (lambda () (keymap-unset combobulate-css-map "M-(")))
      (add-hook 'combobulate-json-minor-mode-hook
                (lambda () (keymap-unset combobulate-json-map "M-(")))
      (add-hook 'combobulate-html-minor-mode-hook
                (lambda () (keymap-unset combobulate-html-map "M-(")))
      :custom
...
#+begin_src emacs-lisp
  ...
  (use-package eglot
    ...
    (defvar-keymap eglot-prefix-map
      :doc "Keymap for the eglot-prefix in prog-mode."
      :prefix 'eglot-prefix-map
      "z" #'eglot)
    (keymap-set prog-mode-map "<f6> e" eglot-prefix-map)
    ...
    Useful when enabling or disabling `eglot'."
      (interactive)
      (if (and (featurep 'flycheck)
               (and flycheck-mode (not eglot--managed-mode)))
          (flycheck-mode -1)
    ...
    (defvar-keymap eglot-prog-map
      :doc "Keymap for eglot-prog commands."
      :prefix 'eglot-prog-map
      "h" #'eglot-find-implementation
      "t" #'eglot-find-declaration
      "n" #'eglot-find-typeDefinition
      "r" #'eglot-rename
      "TAB" #'eglot-format
      "u" #'eglot-code-actions
      "v" #'eglot-reconnect
      "Z" #'eglot-shutdown-all
      "z" #'eglot-shutdown)
    (keymap-set eglot-mode-map "<f6> e" eglot-prog-map))
...
#+begin_src emacs-lisp
  ...
  (use-package xref
    :init
    (defvar-keymap xref-prefix-map
      :doc "Keymap for xref commands."
      :prefix 'xref-prefix-map
      "h" #'xref-find-definitions
      "d" #'xref-find-definitions-other-window
      "n" #'xref-find-references
      "r" #'xref-find-references-and-replace
      "m" #'xref-find-apropos
      "v" #'xref-query-replace-in-results
      ;; Navigating
      "c" #'xref-go-back
      "t" #'xref-go-forward
      "w" #'xref-pop-marker-stack
      ;; Tags
      "u" #'tags-search
      "p" #'tags-query-replace
      "e" #'list-tags)
    (keymap-set prog-mode-map "<f6> u" xref-prefix-map)
    :bind (:repeat-map xref-prog-repeat-map
                       ("h" . xref-find-definitions)
                       ("c" . xref-go-back)
                       ("t" . xref-go-forward)
                       ("w" . xref-pop-marker-stack)))
...
#+begin_src emacs-lisp
  ...
  (use-package flycheck
    :init
    (defvar-keymap flycheck-prefix-map
      :doc "Keymap for flycheck-prefix in prog-mode."
      :prefix 'flycheck-prefix-map
      "z" #'flycheck-mode)
    (keymap-set prog-mode-map "<f6> p" flycheck-prefix-map)

    (defun jb-use-eslint-from-node-modules-flycheck ()
    ...
    (defun jb-use-stylelint-from-node-modules-flycheck ()
      (let* ((root (locate-dominating-file
                    (or (buffer-file-name) default-directory)
                    "node_modules"))
             (stylelint (and root
                             (expand-file-name "node_modules/stylelint/bin/stylelint.mjs"
                                               root))))
        (when (and stylelint (file-executable-p stylelint))
          (setq-local flycheck-css-stylelint-executable stylelint))))
    (defun rf-use-stylelintrc-from-root-project ()
      (let* ((root (locate-dominating-file
                    (or (buffer-file-name) default-directory)
                    "node_modules"))
             (stylelintrc (and root
                               (expand-file-name "stylelint.config.mjs"
                                                 root))))
        (when stylelintrc
          (setq-local flycheck-stylelintrc stylelintrc))))

    :hook ((flycheck-mode . (lambda ()
                              (jb-use-eslint-from-node-modules-flycheck)
                              (jb-use-stylelint-from-node-modules-flycheck)
                              (rf-use-stylelintrc-from-root-project))))
    :bind (:map flycheck-error-list-mode-map
                ("c" . flycheck-error-list-previous-error)
                ("t" . flycheck-error-list-next-error)
                :repeat-map flycheck-prog-repeat-map
                ("c" . flycheck-previous-error)
                ("t" . flycheck-next-error))
    :custom
    ;; Set the executable and config file of the html-tidy syntax checker.
    (flycheck-html-tidy-executable "/usr/bin/tidy")
    (flycheck-tidyrc "/etc/tidy.conf")
    :config
    (defvar-keymap flycheck-prog-map
      :doc "Keymap for flycheck commands."
      :prefix 'flycheck-prog-map
      "c" #'flycheck-previous-error
      "t" #'flycheck-next-error
      "g" #'flycheck-first-error
      "p" #'flycheck-list-errors
      "u" #'flycheck-explain-error-at-point
      "o" #'flycheck-clear
      "w" #'flycheck-compile
      "v" #'flycheck-buffer)
    (keymap-set flycheck-mode-map "<f6> p" flycheck-prog-map))
  ...
  ;; Implements a minor-mode for displaying errors from Flycheck
  ;; right below their reporting location, using overlays
  (use-package flycheck-inline
    :defer t)
  ;; You can enable flycheck-inline by enabling flycheck-inline-mode
...
#+begin_src emacs-lisp
  ...
  (use-package flymake
    :init
    (defun rf-set-eglot-flymake-backend ()
      "Set 'eglot' as the backend for 'flymake'."
      (interactive)
      (setq-local flymake-diagnostic-functions '(eglot-flymake-backend)))
    (defvar-keymap flymake-prefix-map
      :doc "Keymap for the flymake-prefix in prog-mode."
      :prefix 'flymake-prefix-map
      "z" #'flymake-mode)
    (keymap-set prog-mode-map "<f6> ." flymake-prefix-map)
    :bind (:repeat-map flymake-prog-repeat-map
                       ("c" . flymake-goto-prev-error)
                       ("t" . flymake-goto-next-error))
    :config
    (defvar-keymap flymake-prog-map
      :doc "Keymap for flymake commands."
      :prefix 'flymake-prog-map
      "c" #'flymake-goto-prev-error
      "t" #'flymake-goto-next-error
    ...
    (keymap-set flymake-mode-map "<f6> ." flymake-prog-map)
  ...
  ;; A Flymake backend for Js/Ts (flymake-eslint replacement):
  (use-package flymake-jsts
    ...
    (defun jb-use-eslint-from-node-modules-flymake ()
    ...
    eslint executable from that directory if any exists."
      ...
      (let* ((root (locate-dominating-file
        ...
        (when (and eslintx (file-executable-p eslintx))
  ...
  (use-package flymake-stylelint
    ...
    (defun jb-use-stylelint-from-node-modules-flymake ()
  ...
  ;; Locally, meaning you have run npm install stylelint in a project
  ;; and stylelint is present in your package.json file
  ;; (setq flymake-stylelint-use-global nil)
...
#+begin_src emacs-lisp
  ...
  (use-package eldoc
    :init
    (defvar-keymap eldoc-prefix-map
      :doc "Keymap for eldoc comands."
      :prefix 'eldoc-prefix-map
      "i" #'eldoc
      "h" #'eldoc-doc-buffer
      "z" #'eldoc-mode)
    (keymap-set prog-mode-map "<f6> i" eldoc-prefix-map)
...
#+begin_src emacs-lisp
  ...
  (use-package dape
    ...
    :bind (:map dape-global-map
                ;; To prevent these keys from appearing on repeat-map
                ("<" . nil) (">" . nil) ("B" . nil) ("D" . nil) ("M" . nil)
                ("R" . nil) ("S" . nil) ("b" . nil) ("c" . nil) ("d" . nil)
                ("e" . nil) ("f" . nil) ("h" . nil) ("i" . nil) ("l" . nil)
                ("m" . nil) ("n" . nil) ("o" . nil) ("p" . nil) ("q" . nil)
                ("r" . nil) ("s" . nil) ("t" . nil) ("u" . nil) ("w" . nil)
                ("x" . nil))
    :hook (dape-display-source . eldoc-mode)
    :config
    ;; Set here to avoid conflicting with the default settings
    (defvar-keymap dape-prefix-map
      :doc "Keymap for the dape-prefix in prog-mode."
      :prefix 'dape-prefix-map
      "z" #'dape
      "v" #'dape-restart
      "Z" #'dape-kill
      ;; Navigating
      ...
      "j" #'dape-many-windows
      "q" #'dape-select-stack)
    (keymap-set prog-mode-map "<f6> v" dape-prefix-map)
    (defvar-keymap dape-repeat-map
      :doc "Keymap for debugger repeat commands."
      :repeat (:exit (dape-kill
                      dape-quit
...
#+begin_src emacs-lisp
  ...
  ;; It's also used in `grep-mode-map'
  (with-eval-after-load 'compilation
    (keymap-set compilation-minor-mode-map "C-S-t" 'compilation-next-error)
    (keymap-set compilation-minor-mode-map "C-S-c" 'compilation-previous-error)
    ;; (keymap-set compilation-minor-mode-map "" 'compilation-previous-file)
    ;; (keymap-set compilation-minor-mode-map "" 'compilation-next-file)
    (keymap-set compilation-minor-mode-map "c" 'next-error-no-select)
    (keymap-set compilation-minor-mode-map "t" 'previous-error-no-select))
...
#+begin_src emacs-lisp
  ...
  (use-package sgml-mode
    :hook ((html-mode . electric-pair-local-mode)
  ...
  (use-package template-literals-ts-mode
    ;; This needs to be in this order to enable combobulate, and
    ;; template-literals to work together without any issues
  ...
  ;; The `html-ts-mode' bindings.
  (with-eval-after-load 'html-ts-mode
    (keymap-set html-ts-mode-map "<f6> e" eglot-prefix-map)
    (keymap-set html-ts-mode-map "<f6> u" xref-prefix-map) ; remove it
    (keymap-set html-ts-mode-map "<f6> p" flycheck-prefix-map)
    (keymap-set html-ts-mode-map "<f6> ." flymake-prefix-map)
    (keymap-set html-ts-mode-map "<f6> i" eldoc-prefix-map)
    (keymap-set html-ts-mode-map "<f6> z" 'combobulate-mode))

  ;; The `mhtml-mode' bindings.
  (with-eval-after-load 'mhtml-mode
    (keymap-set mhtml-mode-map "<f6> e" eglot-prefix-map)
    (keymap-set mhtml-mode-map "<f6> u" xref-prefix-map) ; remove it
    (keymap-set mhtml-mode-map "<f6> p" flycheck-prefix-map)
    (keymap-set mhtml-mode-map "<f6> ." flymake-prefix-map)
    (keymap-set mhtml-mode-map "<f6> i" eldoc-prefix-map)
    (keymap-set mhtml-mode-map "<f6> z" 'combobulate-mode))

  ;; The `html-mode' bindings.
  (with-eval-after-load 'html-mode
    (keymap-set html-mode-map "<f6> e" eglot-prefix-map)
    (keymap-set html-mode-map "<f6> u" xref-prefix-map) ; remove it
    (keymap-set html-mode-map "<f6> p" flycheck-prefix-map)
    (keymap-set html-mode-map "<f6> ." flymake-prefix-map)
    (keymap-set html-mode-map "<f6> i" eldoc-prefix-map)
    (keymap-set html-mode-map "<f6> z" 'combobulate-mode))
...
#+begin_src emacs-lisp
  ...
  (use-package css-mode
    :hook (css-ts-mode . electric-pair-local-mode)
    :custom
    (css-indent-offset 2))
...
#+begin_src emacs-lisp
  ...
  (use-package js
    :hook (js-base-mode . electric-pair-local-mode)
    :bind (:map js-base-mode-map
                ("<f6> j" . rf-js-save-eval-print)
                ("<f6> q" . rf-js-console-log-wrap))
    :config
    ;; Remove highlighting combobulate in js console functions.
    (custom-set-faces '(combobulate-query-highlight-serene-shade-face
    ...
    (defun rf-js-save-eval-print (&optional output-buffer)
      ...
      (interactive "P")
      (save-buffer)
      (shell-command (concat "node " (buffer-file-name)) output-buffer))
...
#+begin_src emacs-lisp
  ...
  (use-package emacs
    ...
    :bind (:map lisp-interaction-mode-map
                ...
                ("<f6> ;" . transpose-sexps)
                ("<f6> ," . raise-sexp)
                ("<f6> o" . backward-kill-sexp)
                ("<f6> k" . kill-sexp)
                ("<f6> h" . backward-sexp)
                ("<f6> n" . forward-sexp)
                ("<f6> t" . prog-indent-sexp)
                ("<f6> e" . eval-print-last-sexp)
                :repeat-map emacs-lisp-move-repeat-map
                ("h" . backward-sexp)
                ("n" . forward-sexp)
                :repeat-map emacs-lisp-remove-repeat-map
                (";" . transpose-sexps)
                ("," . raise-sexp)
                ("o" . backward-kill-sexp)
                ("k" . kill-sexp))
    :config
    ;; DeepSeek: Force enable in all future scratch buffers
    (defun ai-enable-electric-pair-in-scratch ()
      "Enable electric-pair-mode in scratch buffer."
      (when (string= (buffer-name) "*scratch*")
        (electric-pair-local-mode 1)))
    (add-hook 'after-init-hook 'ai-enable-electric-pair-in-scratch))
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'esh-mode
    (keymap-set eshell-mode-map "C-d" 'pcomplete-list))

  (with-eval-after-load 'em-hist
    (keymap-unset eshell-hist-mode-map "TAB")
    (keymap-unset eshell-hist-mode-map "M-n")
    (keymap-unset eshell-hist-mode-map "M-p")
    (keymap-unset eshell-hist-mode-map "M-r")
...
#+begin_src emacs-lisp
  ...
  ;; rmail-summary-mode bindings
  (with-eval-after-load 'rmailsum
    (keymap-set rmail-summary-mode-map "c" 'rmail-summary-previous-msg)
    (keymap-set rmail-summary-mode-map "n" 'rmail-summary-continue)
    (keymap-set rmail-summary-mode-map "t" 'rmail-summary-next-msg)
    (keymap-set rmail-summary-mode-map "p" 'rmail-summary-toggle-header)
    (keymap-set rmail-summary-mode-map "M-n" nil)
    (keymap-set rmail-summary-mode-map "M-i" 'rmail-summary-next-all)
    (keymap-set rmail-summary-mode-map "M-s" nil)
    (keymap-set rmail-summary-mode-map "M-e" 'rmail-summary-search))
...
#+begin_src emacs-lisp
  ...
  (use-package google-translate
    ...
    (keymap-global-set "<kp-decimal>" 'google-translate-at-point))
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'help-mode
    (keymap-set help-mode-map "c" 'previous-line)
    (keymap-set help-mode-map "t" 'next-line)
    (keymap-set help-mode-map "H" 'help-customize)
    (keymap-set help-mode-map "w" 'scroll-up-command)
    (keymap-set help-mode-map "m" 'scroll-down-command))
