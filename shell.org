...
*** npm install
If a project has a package.json file, it will install everything the project needs, in the node_modules folder, creating it if it's not existing already.
...
[ /-g/ global, without associating it with the project /-D/ or /--save-dev/ installs and adds the entry to the package.json file devDependencies /--no-save/ installs but does not add the entry to the package.json file dependencies /-save-optional/ installs and adds the entry to the package.json file optionalDependencies /--no-optional/ will prevent optional dependencies from being installed /-P/ or /--save-prod/ as a production dependecy (default) (dependences) /install/ or /i/ or /add/ install ]
( ~npm i name~ equivalent command )
( ~npm install name@version~ install a specific version of a package (won't allow upgrades to other versions) )
( ~npm install --save-prod name~ switch installed package from development to production )

*** npm update
Update. npm will check all packages for a newer version that satisfies your versioning constraints.
( ~npm update name~ specify a single package to update )
...
*** npm run task
The package.json file supports a format for specifying command line tasks that can be run by using this command.
...
*** npx
Executes packages without installing them.
...
** font-tools
*** woff2_compress input-font.ttf
Convert a TrueType font (.ttf) to WOFF2. Need to install the woff2 package.
( ~libwoff1~ converting fonts to WOFF 2.0 )

*** sfnt2woff-zopfli input-font.ttf
Create WOFF files with Zopfli compression.
( ~ttf2woff input-font.ttf output-font.woff~ convert a TrueType font (.ttf) to WOFF )

*** fontforge
For converting from ttf to svg.
...
[ /--no-edit/ will allow you to make the amendment to your commit without changing its commit message /--all/ automatically stage files that have been modified and deleted, but new files you have not told Git about are not affected ]
...
| /r1\¦r2/       | /r1¦r2/   | match 1 of regex identified by r1 or r2   |
| /\(r1\¦r2\)/   | /(r1¦r2)/ | match 1 of regex identified by r1 or r2   |
|              |         | (¦=pipe) and treat it as a bracketed rege |
