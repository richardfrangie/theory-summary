...
** Practice
*** New
**** +Sin embargo, tiene la fuerza para describir de forma única una estructura con un dominio infinito,
however, has the strength to uniquely describe a structure with an infinite domain,

**** +La cuarta edición propuesta de ECMA-262 habría sido la primera actualización importante de ECMAScript desde que la tercera edición en 1999 fue publicada
The proposed fourth edition of ECMA-262 would have been the first major update to ECMAScript since the third edition was published in 1999.

**** +El propósito de las pruebas unitarias no es verificar si funciona, es asegurarse de que siga funcionando cuando tú cambia algo.
The purpose of of unit tests isn't to check whether it works, it is to make sure it keeps working when you change something.

**** +Se intenta buscarlo en el entorno padre, en el padre del padre, y así sucesivamente, hasta que se considera toda la cadena de entornos.
there is an attempt to lookup it in the parent environment, in the parent of the parent, and so on — until the whole environment chain is considered.

**** +Durante los dos últimos años, el lenguaje principal promocionado ha sido rust, como todas las veces.
for the last couple years the main hyped language has been rust like every single time

**** +Así que voy a utilizar las dos métricas diferentes para intentar definir el lenguaje actual tan publicitado.
so I'm going to use the two different metrics to try to Define hyped up current language

**** +La primera métrica va a ser la cantidad de nuevas preguntas de Stack Overflow que son etiquetadas con un lenguaje de programación en particular.
the first metric is going to be the number of new stack Overflow questions that are tagged with a particular programming language

**** +Y la segunda métrica que voy a utilizar es la cantidad de nuevos miembros que se unen al subreddit de un idioma en particular.
and the second metric that I'm going to use is the number of new members joining a particular language's subreddit

**** +Y voy a hacer esto en base a porcentajes.
and I'm going to do this based of percentages

**** +En realidad, va a tener menor porcentaje de crecimiento que Zig, que es otro
it's actually going to have less percentage of growth than Zig which is another

**** +Creo que para finales de 2025, al menos dos importantes plataformas de streaming van a utilizar...
I think that by the end of 2025 at least two major streaming platforms are going to use

**** +La razón por la que es tan atractivo para las plataformas de streaming es porque ahorra una cantidad significativa de ancho de banda para aproximadamente el mismo nivel de calidad.
the reason it's so appealing to a streaming platforms is because it saves a significant amount of bandwidth for about the same level of quality

**** +Creo que hay una posibilidad muy pequeña, tal vez un 30% de que un importante proveedor de streaming se asocie con un proveedor de televisión.
I think there is a real small chance maybe like 30% chance that a major streaming vendor pairs up with a TV vendor

**** +Ahora quiero hablar un poco sobre la compresión.
now I want to talk a little bit about compression
...
**** +Y un área donde funciona realmente bien es al escalar animes más antiguos que podrían no haber sido lanzados en HD, por ejemplo.
and one area where it works really well is upscaling older animes that might not have been released in HD for example

**** +Así que, básicamente, si la calidad es realmente mala, puedes mejorarla.
so basically if the quality is really bad you can upscale it

**** +Así que tienes que ir y habilitarlo si quieres probarlo.
so you have to go through and enable it if you want to try out

**** +Y realmente funciona sorprendentemente bien
and it actually works surprisingly well

**** parte de mi proceso de pensamiento detrás de esto es que la mayoría de los lenguajes de programación exagerados tienen alrededor de 3 a 5 años de tiempo de crecimiento
part of my thought process behind this is that most hyped up programming languages have about a 3 to 5 year time of growth

**** En realidad está alcanzando el final de su máximo tiempo publicitado.
is actually reaching the kind of end of its maximum hyped time

**** Por ejemplo, Ruby definitivamente tuvo un período de auge y alcanzó su máximo potencial.
for example Ruby definitely had a period of boom time and reached their maximum

**** Yo diría que la capacidad de auge después de esa cantidad de tiempo haya pasado
I would say boom capacity after that amount of time has passed

**** Puede que no lo sepas, pero la mejora/ampliacion/escalada de la IA existe desde hace bastante tiempo
so you might not be aware but AI upscaling has been around for a pretty decent amount of time

**** Y lo hace parecer casi como si fuera lanzado en HD, lo cual es bastante bueno
and it makes it look almost like it was released in HD which is pretty nice

**** También hay otra característica que probablemente no conozcas y que es parte de Mi y es
there is also another feature that you are probably not aware of that's part of Mi and there is

**** pero está integrado y establecida de forma predeterminada
but it's built into it and set off by default

**** pero funcionará en tus videos y se aplicará quizás a un video de YouTube que estés viendo si
but it's it'll work on your videos and apply to maybe a YouTube video you're watching if

**** Si tiene la calidad configurada muy mal o es un video de muy baja calidad, aumente la escala del video automáticamente.
if you have the quality set really bad or it's a super low quality video to automatically upscale the video

**** así que creo que va ser una característica realmente interesante en el futuro.
so I think this is going to be a really interesting feature going forward

**** Esta asociación se mantiene sin cambios.
This association further stays unchanged.

**** Este es un buen lugar para hacerse una idea de lo que Babel puede hacer y ver cómo podría encajar en su vida.
this is a good place to get an idea of what Babel can do and see how it might fit into your life.
