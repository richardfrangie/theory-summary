...
** Practice
*** New
**** +; el desarrollador del programa propietario se ve tentado a diseñar el programa para maltratar a sus usuarios.
; the proprietary program's developer is tempted to design the program to mistreat its users.

**** +Ceder a esa tentación se ha vuelto cada vez más frecuente
Yielding to that temptation has become ever more frequent;

**** +Mantendremos toda nuestra base de datos de informes de errores abierta a la vista del público en todo momento.
We will keep our entire bug report database open for public view at all times.

**** +y enlaces al manual para temas más avanzados o para cuando algo salga mal.
and links to the manual for more advanced topics or for when things go wrong.

**** +pero también son bastante potentes y un movimiento en falso te puede costar.
; but they are also quite powerful and a false move can cost you.

**** +¿Tienes en tu escritorio algunos archivos antiguos que no has abierto desde hace tiempo?
Have some old files you haven't opened in awhile on your desktop?

**** +Pero antes de dejarnos llevar por la generosidad de Apple,
But before we get too carried away about Apple's generosity

**** +Si eres nuevo en Unix, probablemente deberías salir, comprar algunos libros y leer un poco.
If you are new to Unix, you probably should go out and buy some books and do some reading.

**** +Uno/se selecciona principalmente respuestas ya sea por
one mostly selects answers either by

**** +, podrías intentar adivinar la clave con un ataque de fuerza bruta,
, you could try to guess the key with a brute force attack,

**** +El orden en el que son listados puede diferir del que usted está acostumbrado.
The order they are listed in may differ from what you are used to.

**** +La forma más sencilla de suscribirse a una o más de estas listas es visitar la página de suscripción a la lista de correo de Debian y completar el formulario que encontrará allí.
...
**** +El espacio en disco es barato hoy en día, pero ¿por qué tirar tu dinero?
Disk space is cheap nowadays, but why throw your money away?

**** +(esto)Hará preguntas de configuración, como cuando se instaló el paquete por primera vez.
It will ask configuration questions, much like when the package was first installed.

**** +*Podrás* ir más allá y obtener información respecto la protección de la privacidad,
You will be able to go further and obtain information concerning the privacy protection,

**** +Todo empezó con un pequeño, *pero sólido* grupo de hackers de software libre,
Everything started with a little, but solid, group of free software hackers,

**** +, pero también *contra* tus propios errores
, but also against your own mistakes

**** +así que no dude en probar otros *entornos*.
so don’t hesitate to test other environments.

**** +cualquier tipo de dato o producto que no queramos alterar
any kind of data or product that we don't want tempered with
...
**** +GNU/Linux también tiene menos probabilidades de fallar
GNU/Linux is also less likely to crash,

**** , es posible que pueda arreglárselas con *algo menos* del hardware recomendado que se enumera en la siguiente tabla.
, you might manage with less than some of the recommended hardware listed in the table below.

**** realizar cambios en/a la partición de un disco es una acción intrínsecamente peligrosa y sólo debería realizarse/ser hecha después de haber realizado una copia de seguridad completa de todos los datos.
making changes to the partitioning of a disk is an inherently dangerous action and should only be done after having made a full backup of all data.

**** Está escrito por el mismo autor que el gestor de arranque syslinux y, por lo tanto, es menos probable que cause problemas.
It's written by the same author as the syslinux bootloader and is therefore least likely to cause issues.

**** Tenga en cuenta que el uso del disco varía ampliamente según/en función/dado la configuración del sistema.
Note that disk usage varies widely given system configuration

**** Los datos temporales creados por los programas probablemente *irán* a este directorio.
temporary data created by programs will most likely go in this directory.

**** +La misión de este manual no es ser exhaustiva
The mission of this manual is not to be comprehensive –

**** +para lidiar no solo con espacio de memoria insuficiente, sino también con la funcionalidad de suspensión o hibernación.
, to deal not only with insufficient memory space, but also with the suspend or hibernation functionality.

**** +Debian está desarrollado muy minuciosamente.
Debian is developed very thoroughly.

**** +Pero entonces... ¿cómo vamos a hacerlo/lograrlo/conseguirlo?
But then … how are we going to make it?

**** +La distribución Experimental no es una distribución Debian per se, sino más bien un repositorio donde se prueban las versiones alfa o beta del software.
...
**** +La “nube” está de moda estos días.
The “cloud” is fashionable these days.

**** Uno de los principales intereses del software libre es que permite a personas cualificadas auditar el código del programa,
One of the major interests of the free software is that it allows skilled people to audit the program code,

**** La mayoría de los usuarios nunca han tenido que instalar un sistema previamente, y eso podría ser un poco aterrador
Most of the users never had to install a system previously, and that could be a little freaking

**** si hubiera optado por un conjunto de CD de Debian
Had you opted for a Debian CD set - If you had opted for a set of Debian CD's

**** La wiki oficial de la comunidad está muy bien hecha y te brindará/provee una ayuda eficiente y completa (no es necesario reinventar la rueda).
The official wiki of the community is very well done and will provide you with an efficient and complete help (no need to reinvent the wheel

**** La ventana principal aparece y revela... muchas herramientas con las que divertirse...
The main window pops up and unveils … lots of tools to have fun with …

**** En primer lugar, hay que saber/uno debe saber que existen(verb) varias ramas de la distribución Debian en paralelo.
First of all, one must know that several Debian distribution branches exist in parallel.

**** cuando se incluyen todas las funciones buscadas/objetivas/apuntadas por los equipos de Debian
when all the features targeted by the Debian teams are included,

**** y después de un período de congelación del software y búsqueda/caza de errores(bug),
and after a period of software freeze and bug hunting,

**** Quizás pienses *que* estas informaciones son aparentemente inofensivas,
You may think that these information are seemingly harmless,

**** o simplemente compartir tus recursos, le sugerimos encarecidamente que cree otra cuenta de usuario
or simply share your resources, we strongly suggest you create another user account

**** +Cuanto más segura/fuerte sea mi contraseña, más difícil es memorizarla. ¿No es así?
 The stronger my password is, the more difficult it is to memorize it. Isn’t it ?

**** los datos de los demás usuarios son legibles por usted, y los suyos también,
the data of the other users are readable by you, and yours too

**** *¡Y esto no es en vano! ... y adquiera el hábito de guardar sus datos en un medio externo y de forma regular.
 and this is not for nothing! ... and get into the habit of saving your data on an external medium and on a regular basis.

**** *Seguramente te topaste con estos mensajes de advertencia que hablaban de hacer una copia de seguridad de tus datos...
You certainly ran into these warning messages talking about backing up your data …

**** , siempre *dejarás* un rastro detrás de ti.
you will always leave a trail behind you.

**** No puede garantizar su anonimato cuando se enfrenta a alguna tecnología de seguimiento avanzada como las huellas dactilares
 It cannot guarantee your anonymity when facing some advanced tracking technology like the fingerprinting

**** Deja de hacer como todos los demás, aunque/incluso supuestamente “no tengas nada que ocultar”.
Stop doing like everyone else, even if supposedly “you have nothing to hide”.

**** Esta funcionalidad no debe impedirle mantener algo de sentido común..
This functionality must not prevent you to keep some common sense

**** Corresponde a cada uno definir su “vida privada” y no difundirla/esparcirla en la red…
It is up to everyone to define his “private life” and not spread it on the web

**** La Fundación Linux se asocia con los principales desarrolladores del mundo
The Linux Foundation partners with the world’s leading developers

**** Imagínese un mundo en el que cada ser humano pueda compartir libremente la suma de todo el conocimiento. Ese es nuestro compromiso
Imagine a world in which every single human being can freely share in the sum of all knowledge. That’s our commitment.

**** , Wikimedia se esfuerza por
, Wikimedia strives to

**** con un conjunto completo de herramientas para desarrolladores y creadores de todo tipo.
with a complete set of tools for developers and makers of all kinds.

**** Tenía mucho tiempo que no entraba a Facebook. En estos días entre en facebook y me aparecieron un montón de vídeos tuyos. Tenían un montón de vistas, cientos de miles, pero lo que me molestó mucho fue, que ellos cortan la pantalla inferior y desaparecen tu logotipo. No me gustan las marcas de agua, pero usted debería ponerla un poco más alto.
I haven't been on Facebook for a long time. I went on Facebook few days ago and a bunch of your videos appeared. They had a lot of views, hundreds of thousands, but what really bothered was that they cut off the bottom screen and disappear your logo. I don't like watermarks, but you should put it a little higher.
